...
class SMerchantRepository extends RepositoryBase
{
    ...
    public function getSearchList($skip, $limit,$type,$keywords)
    {
        ...
        if($type==2){
            $params[':name'] = $keywords;
            $sql="SELECT SQL_CALC_FOUND_ROWS a.name, a.logo, a.id, a.address,a.summary, b.name AS nametype,b.app_code FROM s_merchant a LEFT JOIN seller_type b ON a.type_pid = b.id WHERE a.is_deleted = 0 and a.name like CONCAT('%', :name, '%') ORDER BY a.created_on desc LIMIT :skip,:limit";
        }
       if($type==3){
