...
class WeixinPayService extends ServiceBase{
     ...
     * 获取公众号管理站点
     */
  /*  public function getPublicAccountSite(){
        $publicAccountSite = \app\framework\biz\cache\SiteCacheManager::getSiteUrl('MemberCenterSite');
        if (empty($publicAccountSite)) {
            throw new \Exception('缺少MemberCenterSite配置项');
        }
        return $publicAccountSite;
    }
    /*
     * 获取公众号管理站点
     */
    /*public function getPassportSite(){
        $PassportSiteSite = \app\framework\biz\cache\SiteCacheManager::getSiteUrl('PassportSite');
        if (empty($PassportSiteSite)) {
            throw new \Exception('缺少PassportSite配置项');
        }
        return $PassportSiteSite;
    }*/
    /**
     * 获取接口的access_token
     * @return string
   ...
   /*
    ...
    public function getPayKey($public_id){  
        ...
        try {
            $params = [
                'access_token' => $this->getAccessToken($public_id),
                'account_id' => $public_id
            ]; 
            $params = http_build_query($params);
            $client = new RestClientHelper($this->getManageCenterSite());
            $result = $client->invoke("/api/public-account/get-mch/?" . $params, [], 'GET');
            $res = $this->getResult($result);
            if (empty($res->mch_half_key) || strlen($res->mch_half_key) != 16) {
                throw new \Exception('获取公众号商户密钥失败！');
            }
            return $res->mch_half_key;
        } catch (\Exception $ex) {
