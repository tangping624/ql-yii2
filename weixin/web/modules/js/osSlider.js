...
function osSlider(objs) {
    ...
    that.width = document.body.offsetWidth;//得到容器的宽度
    that.height =  parseFloat(document.body.offsetWidth)/2.8;//得到容器的高度
    ...
    that.cNodes.find('img').css({
        ...
        // });
        //为高亮导航创建节点
        var $navParent = $('<ul class="slider-nav" style="display:none;"></ul>');
    ...
    that.moveSwitch = function(mid,command,tid) {
        ...
        switch (mid) {
            case 0:
                ...
                break;
            case 1:
                that.cellToggle(tid,1);
                break;
            case 2:
                that.gridLeft(tid,2);
                break;
            case 3:
                that.cellToggle(tid,0);
                break;
            case 4:
                that.gridLeft(tid,1);
                break;
            case 5:
                that.cellToggle(tid,2);
                break;
            case 6:
                that.gridLeft(tid);
                break;
    ...
    that.gridTop = function(tid,showNum) {
        ...
        for (var i = 0; i < 12; i++) {//利用循环 创建出栅格节点
            ...
            // console.log($cvNode.find('*'));
        }
        console.log($cvNode.find("img").css("width"));
        //分配对应效果
        switch (showNum) {
            default:
            case 0:
