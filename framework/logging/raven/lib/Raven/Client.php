...
class Raven_Client
{
    ...
    public function __construct($options_or_dsn=null, $options=array())
    {
        ...
        $vars = Raven_Util::get($options,'log_vars', []);
        $this->log_vars = empty($vars) ? ['_COOKIE', '_SESSION', '_SERVER'] : $vars;
    ...
    protected function should_log_var($var_name){

        if(empty($var_name)){
            throw new \InvalidArgumentException('$var_name');
        }

        if(in_array($var_name, $this->log_vars)){
            return true;
        }
        return false;
    }
    ...
    {
        $env = $headers = array();
        if($this->should_log_var('_SERVER')){
        ...
        if($this->should_log_var('_COOKIE')){
            if (!empty($_COOKIE)) {
    ...
    public function capture($data, $stack, $vars = null)
    {
        ...
        if($this->should_log_var('_SESSION')){
