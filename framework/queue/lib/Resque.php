...
class Resque
{
	...
	public static function redis()
	{
		...
		if(is_array($server)) {
			...
			$uid = isset($dsn['user']) ? $dsn['user'] : '';
			$pwd = isset($dsn['pass']) ? $dsn['pass'] : '';
			require_once dirname(__FILE__) . '/Resque/Redis.php';
			self::$redis = new Resque_Redis($host, $port);
			if ($uid && $pwd) {
				\Yii::trace('redis身份认证,' . $server);
				if (!self::$redis->auth("{$uid}:{$pwd}")) {
					throw new \Exception('redis身份认证失败! uid:' . $uid);
				}
			}
		}
	...
	{
		if (!is_string($queue)) {
			throw new \InvalidArgumentException('无效的参数类型, expect string, but ' . json_encode($queue));
		}
	...
	public static function pop($queue)
	{
		...
		if (!is_string($item)) {
			throw new \InvalidArgumentException('$item必须是string类型：' . json_encode($item).'。提供的类型：'.gettype($item).'。queue:'.$queue);
		}
