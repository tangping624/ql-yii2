...
class Resque_Worker
{
	 ...
	 * @param array $excludeQueues
	 */
	public function __construct($queues, $excludeQueues = [])
	{
		if (!is_array($queues)) {
			$queues = array($queues);
		}
	...
	public function queues($fetch = true)
	{
		...
		return $this->filterQueues($queues);
	}
	 ...
	 * @return array
	 */
	public function filterQueues($queues)
	{
		if (empty($queues) || empty($this->excludeQueues)) {
			return $queues;
		}
		return array_filter($queues, function ($item) {
			return !in_array($item, $this->excludeQueues);
		});
