...
//  模板
    ...
    var $imgWraps = $('#img_wraps'),
     ...
     var _uploader = null, _validate,_city,_type,_initDate = null,
        $deleteImgWrap,   $editName,    $deleteUeWrap, _UEArr = [], 
         _isEdit = $('#is_edit').val(),
         // sellerid=$("#seller_id").attr("data-seller"),
    ...
    var _bindEvent = function() {
         ...
         $(".glyphicon-search").click(function(e){
            // var sell=$("#seller_id").attr("data-seller");
            e.stopPropagation();
             page++
            if(_isEdit&&page==1){
                 $("#seller_id").val("");
        ...
        shopGrid.find('tbody').html('<tr><td colspan="'+shopGrid.find('thead tr.on th').length+'"class="align-c" style="height:70px;">正在加载数据...</td></tr>');
           ...
           shopGrid.grid({
                ...
                getReadURL : function(){
                    var strurl = "/house/house/ajax-seller-list";
         ...
         loaded();})
            ...
            //       $("input[name='seller_id']").val($("input[type='checkbox'][checked]").closest("tr").find(".name_check").html());
            //       $("input[name='seller_id']").attr("data-seller",$("input[type='checkbox'][checked]").attr("data-id"));
            //       $(".products").show();
            //       var id=$("[type='checkbox'][checked]").attr("data-id");
            //       O.ajaxEx({
            //         type: 'get',
            //         url: O.path('/house/house/ajax-house-type?id='+id),
            ...
            if(seller){
             O.ajaxEx({
                    type: 'get',
                    url: O.path('/house/house/ajax-house-type?id='+seller),
                    success: function(data) {
                        // console.log(data);
                        $(".products").show();
                        var html='';
                        var typeid=$("#type_id").attr("data-type");
                        if(data.length>0){
            ...
            $(document).on('click',function(e){
                ...
                // console.log($(elem).closest(".shop_grid").length);
                 if ($(elem).closest(".shop_grid").length) {  
         ...
         $('.submit_btn').on('click', function() {
            if(_doCheck()&&$(this).hasClass("next")) {
                ...
                O.ajaxEx({
                    type: 'post',
                    data: data,
                    url: O.path('/house/house/save'+ (_isEdit ? '&id=' + $('#newsId').val() : '')),
                    success: function(data) {
                        if(data.result == true){
                            ...
                            // $(window).off('beforeunload');
                            // location.href = '/index.php?r=house/house/index'
                        }else{
                            _showTips(data.msg);
    ...
    var _getData = function() {
        ...
        // seri.type_id=seri.type_id?seri.type_id:'';
        seri.content=ueCon;
