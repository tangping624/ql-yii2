...
define(function (require, exports, module) {
    ...
    var load=function(queryType){
        houseGrid.find('tbody').html('<tr><td colspan="' + houseGrid.find('thead tr.on th').length + '" class="align-c" style="height:70px;">正在加载数据...</td></tr>');
        if(cache.view){
            cache.view.search();
        }else{
            cache.view=houseGrid.grid({
                idField : 'id',
                ...
                getReadURL : function(){
                    var strurl = "/house/house/ajax-house-list";
    ...
    var bindEvent=function() {
        ...
        $('body').on('click', '.tips-wrap .deleted-oper', function () {
            ...
            if ($this.hasClass('bg-green')) {
                O.ajaxEx({
                    type: 'get',
                    url: O.path('/house/house/deleted?id=' + curid),
