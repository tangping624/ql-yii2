...
define(function (require, exports, module) {
    ...
    textTips.prototype = {
		...
		checkTextOverflow:function($el){
			var font_size = $el.css('font-size'),
				font_weight = $el.css('font-weight'),
				text = $el.text(),
				el_w = $el.width(),
				html = $('<div style="display:inline-block;font-size:'+ font_size +';font-weight:'+ font_weight +';">'+ text +'</div>');
			$('body').append(html);
			var check_w = html.width();
			html.remove();
			if(check_w > el_w){
				return true; // 文本溢出
			}else{
    ...
    $(document).mousemove(function(e) {
        ...
        if($el.attr('data-title')){
            ...
            if(tip.targetEl != e.target){
				if($el.attr('data-title-auto') && !tip.checkTextOverflow($el)){
					return false;
				}
