...
define(function (require, exports, module) {
    ...
    var SelectProto = {
        ...
        change: function (instance, option, oldOption) {
            ...
            if (instance.originSelectSource) {
                var node = config.node,
                    optionEl = option ? node.find('option[value="' + option[keys.value] + '"]') : [],
                    oldOptionEl = oldOption ? node.find('option[value="' + oldOption[keys.value] + '"]') : [];
        ...
        setSelected: function (instance, selected) {
            ...
            instance.text.val(text).attr('value', text).show().toggleClass('placeholder', !!option.__placeholder);

            if(!isFirst){
                this.change(instance, option, oldOption);
            }
            return true;
        },
        ...
        create: function (instance) {
            var config = instance.config, $node = config.node,
                options = config.options = config.options || [];

            //初始化
            var isOrigin = this._parseOptions(instance, config, options);

            //初始化wrap
            instance.wrap = this.createWrap($node, isOrigin || !$node.is('div'), config);
            ...
            if (!options.length) {
                if ($node.is('select')) {
                    ...
                    optionEls.each(function (i) {
                        ...
                        option.selected = i ? optionEl.prop('selected') : false;
                    ...
                    if (dd.length) {
                        dd.each(function (i) {
                            ...
                            option.selected = i ? !!optionEl.data('selected') : false;
                            _self.fetch(config.dataKey, keys, function (key) {
                                option[key] = optionEl.data(key) || $node.data(key);
                            });
                            if (!config.placeholder || (option[keys.value] && option[keys.text])) {
        ...
        _setPlaceholder: function (config, options, hidePlaceholder) {
            if (config.placeholder) {
                ...
                if (!hidePlaceholder) {
                    if (options.length && options[0][keys.text] === config.placeholder) {
                        return;
                    }
    ...
    $.fn.extend({
        selectBox: function (config) {
            //暂时仅支持单选择器创建
            return create(this.first(), config);
        },
        /**
         * config
         * params
